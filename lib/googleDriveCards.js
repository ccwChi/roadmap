...
// 資料夾結構
const CARDS_FOLDER = 'map_project';
 ...
 * @param {string} deviceId - 修改者的裝置 ID
 */
export async function saveCardsMetadata(cards, deviceId) {
    try {
        // 只儲存元資料，不包含完整內容
        const metadata = {};
        Object.entries(cards).forEach(([id, card]) => {
            metadata[id] = {
                ...
                links: card.links,
                projectId: card.projectId, // 儲存專案歸屬
        ...
        const dataToSave = {
            ...
            lastModifierDeviceId: deviceId, // 記錄是誰修改的
            cards: metadata
        };
        ...
        console.log('[CardSync] 元資料已同步:', Object.keys(metadata).length, '張卡片, By:', deviceId);
        return true;
    } catch (error) {
        console.error('[CardSync] 儲存元資料失敗:', error.message || error);
        console.error('[CardSync] 錯誤詳情:', {
        ...
        if (data && typeof data === 'object') {
            ...
            if (data.cards) {
                ...
                return {
                    ...
                    lastModifierDeviceId: data.lastModifierDeviceId || null // 返回修改者 ID
                };
            }
