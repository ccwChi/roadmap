...
export async function saveCardsMetadata(cards) {
    try {
        ...
        await saveData(filePath, metadata);
 ...
 * @returns {Object} 卡片元資料物件
 */
export async function loadCardsMetadata() {
    try {
        const filePath = `${CARDS_FOLDER}/${METADATA_FILE}`;
        const metadata = await loadData(filePath);

        if (metadata && typeof metadata === 'object') {
            console.log('[CardSync] 元資料已載入:', Object.keys(metadata).length, '張卡片');
            return metadata;
        }
...
/**
 * 完整載入：元資料 + 所有內容
 * @returns {Object} { cards, cardContents }
 */
export async function fullLoad() {
    try {
        // 1. 載入元資料
        const cards = await loadCardsMetadata();
